...
```{r}
ggplot(stressor_summary, aes(x = reorder(stressor, mean_pct), y = mean_pct)) +
...
## Plot 2: Stressor Trends Across Quarters

Next, let's look at how stressor levels change across the four quarters of 2024. We'll compute national averages by stressor and quarter, then use `facet_wrap()` to make a small multiple plot — one panel per stressor:

```{r}
quarterly_summary <- stressors_long |>
  filter(state == "California") |>
  group_by(stressor, quarter_label) |>
  mutate(quarter_label = factor(quarter_label, levels = c("Jan–Mar", "Apr–Jun", "Jul–Sep", "Oct–Dec")))

ggplot(quarterly_summary, aes(x = quarter_label, y = pct_affected, group = stressor)) +
  geom_line(color = "forestgreen", linewidth = 1) +
  geom_point(color = "forestgreen", size = 2.5) +
  facet_wrap(~ stressor, scales = "free_y") +
  labs(
    title = "California Honeybee Colony Stressor \n Levels by Quarter (2024)",
    x = "Quarter",
    y = "Mean % of Colonies Affected",
    caption = "Source: USDA NASS Honeybee Colony Survey"
  ) +
...
`facet_wrap()` is incredibly useful — it splits a plot by a categorical variable so you can compare patterns side by side without cluttering a single panel.
...
- **Visualizing** data with `ggplot2` using bar charts, line plots, and facets
